          ...
          <div className="p-6 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-[10px] text-slate-400 font-medium mt-0.5 uppercase tracking-wider">Configure visual forms bound directly to the Universal Registry</p>
              </div>
            </div>
            {!isReadOnly && (
              ...
              <div className="space-y-4">
                {components.map((comp: any, idx: number) => (
                  <div key={idx} className={`p-4 border rounded-2xl grid grid-cols-12 gap-3.5 items-center transition-all duration-300 ${isReadOnly ? 'bg-transparent border-slate-100' : 'bg-slate-50/40 border-slate-200/60 shadow-sm'}`}>
                    ...
                    <div className="col-span-4"><label className="block text-[9px] font-bold text-slate-400 mb-1 tracking-wide uppercase">Data Binding (Universal Registry)</label><IsoFieldSelector value={comp.field_binding} onChange={(val: string) => { const newC = [...components]; newC[idx].field_binding = val; handleInputChange(setComponents, newC); }} placeholder="Select Backend Bind..." disabled={isReadOnly} /></div>
