...
export const NodePropertiesDrawer: React.FC<NodePropertiesDrawerProps> = ({ node, onClose, onUpdateData }) => {
  ...
  // Read-only state for previously saved steps
  ...
  const handleSaveAll = () => {
    onUpdateData({
      ...
      transitions,
      lastModifiedBy: 'Current User', // Mock user for demonstration
  ...
  return (
    <div className="w-full h-full bg-white/90 backdrop-blur-md border border-slate-200/60 rounded-2xl shadow-glass flex flex-col">
      <div className="flex justify-between items-center px-6 py-4.5 border-b border-slate-100 bg-slate-50/50">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-[10px] text-slate-400 font-bold tracking-widest mt-0.5">ID: {node.id}</div>
          </div>
        </div>
      ...
      {lastModifiedBy && (
        <div className="bg-amber-50 border-b border-amber-100 px-5 py-2 flex items-center justify-between shadow-inner">
           ...
           {isReadOnly && <span className="bg-amber-100 text-amber-800 border border-amber-200 text-[9px] font-bold px-2 py-0.5 rounded uppercase tracking-widest">Read Only</span>}
        </div>
      )}

      <div className="flex-1 overflow-y-auto p-6">
        <div className={`space-y-6 ${isReadOnly ? 'opacity-60 pointer-events-none grayscale-[10%]' : ''}`}>
        <div>
        ...
        {isStudioNode && (
          <div className="pt-4 pb-2 border-b border-slate-100 flex flex-col gap-3">
             <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider">Configured Engine Asset</label>
             <select 
               ...
               className="w-full text-[12px] border border-slate-200 rounded-xl p-2 outline-none font-mono text-indigo-650 bg-white"
             >
               <option value="" disabled>Select Asset to Execute...</option>
            ...
            <button 
              ...
              className="w-full bg-gradient-to-r from-indigo-600 to-indigo-800 hover:from-indigo-700 hover:to-indigo-900 text-white font-extrabold text-[12px] py-3 rounded-xl shadow-lg shadow-indigo-500/20 active:scale-[0.98] transition-all flex items-center justify-center gap-2 mt-1"
            >
              <span>🚀</span> Open Designer Studio
            </button>
          </div>
        )}
        ...
        {!isStudioNode && (
          <div className="pt-5 border-t border-slate-100">
            ...
            <div className="flex items-center gap-2 mb-3">
              <input type="checkbox" id="stpToggle" checked={stpEnabled} onChange={(e) => setStpEnabled(e.target.checked)} className="rounded text-indigo-600 focus:ring-indigo-500" />
              <label htmlFor="stpToggle" className="text-[12px] font-bold text-slate-650 cursor-pointer">Straight-Through Processing (Real-time / STP)</label>
            </div>
            {!stpEnabled && (
            ...
            <div className="space-y-2 mb-3">
              {requiredDocuments.map((doc, idx) => (
                <div key={idx} className="flex justify-between items-center bg-slate-50 border border-slate-150 px-3 py-2 rounded-xl text-[11px] font-semibold text-slate-700">
                  ...
                  <button onClick={() => setRequiredDocuments(requiredDocuments.filter((_, i) => i !== idx))} className="text-rose-600">X</button>
                </div>
              ))}
            </div>
            <div className="flex gap-2">
              <select value={newDocName} onChange={(e) => setNewDocName(e.target.value)} className="flex-1 text-[11px] border border-slate-200 rounded-xl p-2 bg-white">
                <option value="">Select Document...</option>
                {filteredDocuments.map((d: any) => <option key={d.document_id} value={d.document_name}>{d.document_name}</option>)}
              </select>
              <button disabled={!newDocName} onClick={handleAddDocument} className="bg-slate-850 text-white px-3 py-1.5 rounded-xl text-[10px] font-bold">Add</button>
            </div>
          </div>
        )}
        ...
        {!isStudioNode && (
          <div className="pt-5 border-t border-slate-100 flex flex-col gap-2">
            ...
            <div className="flex gap-2">
              <select value={screenTemplate} onChange={(e) => setScreenTemplate(e.target.value)} className="flex-1 text-[12px] text-slate-800 border border-slate-200 rounded-xl p-2 bg-white">
                <option value="">No Screen (Background Task)</option>
                {screenData?.screens?.map((s: any) => <option key={s.screen_id} value={s.screen_id}>{s.screen_name}</option>)}
              </select>
              <button 
      ...
      <div className="p-4.5 border-t border-slate-150 bg-slate-50/50 flex justify-end gap-3">
        ...
        {isReadOnly ? (
          <button onClick={() => setIsReadOnly(false)} className="px-5 py-2.5 text-[13px] font-bold text-slate-700 bg-white border border-slate-300 rounded-xl hover:bg-slate-50 transition-all active:scale-[0.98] shadow-sm flex items-center gap-2">
            <span>✏️</span> Modify Configuration
          </button>
        ) : (
